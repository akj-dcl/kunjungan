...
const form = useForm({
  upt_id: String(props.blok.upt_id ?? ''),
  nama_blok: props.blok.nama_blok ?? '',
})
...
function submit() {
  ...
  })).put(`/admin/bloks/${props.blok.id}`)
}
</script>

<template>
  <Head title="Edit Blok" />

  <AppLayout>
    <div class="flex h-full flex-1 flex-col gap-4 p-4 md:p-6 bg-background text-foreground">
      <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 class="text-2xl font-semibold tracking-tight">Edit Blok</h1>
          <p class="text-sm text-muted-foreground">Perbarui data lokasi blok.</p>
        </div>
