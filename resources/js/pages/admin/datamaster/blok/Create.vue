  ...
  <AppLayout>
    <div class="flex h-full flex-1 flex-col gap-4 p-4 md:p-6 bg-background text-foreground">
      <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 class="text-2xl font-semibold tracking-tight">Tambah Blok</h1>
          <p class="text-sm text-muted-foreground">Buat data lokasi blok baru di UPT.</p>
        </div>
        ...
        <form @submit.prevent="submit" class="p-6 space-y-6">
          <div class="grid gap-6 md:grid-cols-2">
            ...
            <div class="md:col-span-2 space-y-2">
              ...
              <div v-if="form.errors.upt_id" class="text-sm font-medium text-destructive">
                {{ form.errors.upt_id }}
              </div>
            </div>

            <div class="md:col-span-2 space-y-2">
              <label class="text-sm font-medium leading-none">Nama Blok</label>
              <input
                v-model="form.nama_blok"
                type="text"
                placeholder="Contoh: RINJANI, MANDALIKA"
                class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:opacity-50"
              />
              <div v-if="form.errors.nama_blok" class="text-sm font-medium text-destructive">{{ form.errors.nama_blok }}</div>
            </div>
