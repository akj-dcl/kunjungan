...
const form = useForm({
  upt_id: String(props.wbp.upt_id ?? ''),
  no_reg_instansi: props.wbp.no_reg_instansi ?? '',
  nama: props.wbp.nama ?? '',
  alamat: props.wbp.alamat ?? '',
  jenis_kejahatan_id: String(props.wbp.jenis_kejahatan_id ?? ''),
  sel_id: String(props.wbp.sel_id ?? ''),
  is_aktif: props.wbp.is_aktif,
})
  ...
  <AppLayout>
    <div class="flex h-full flex-1 flex-col gap-4 p-4 md:p-6 bg-background text-foreground">
      <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 class="text-2xl font-semibold tracking-tight">Edit WBP</h1>
          <p class="text-sm text-muted-foreground">Perbarui data Warga Binaan Pemasyarakatan.</p>
        </div>
          ...
          <div class="grid gap-6 md:grid-cols-2">
            ...
            <div class="space-y-2">
              ...
              <select v-model="form.upt_id" class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:opacity-50">
                <option v-for="u in upts" :disabled="upts.length === 1" :key="u.id" :value="String(u.id)">{{ u.name }}</option>
              </select>
            ...
            <div class="space-y-2">
              ...
              <input v-model="form.no_reg_instansi" type="text" class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:opacity-50" />
            ...
            <div class="md:col-span-2 space-y-2">
              ...
              <input v-model="form.nama" type="text" class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:opacity-50" />
